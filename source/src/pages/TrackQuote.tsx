          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full border rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[#E60012] uppercase min-h-[44px]"
                required
              />
            </div>
            <button
                ...
                {["new", "processing", "quoted", "accepted"].map((step, idx) => {
                  ...
                  return (
                    <div key={step} className="flex flex-col items-center gap-1 flex-1 relative">
                      <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center text-xs font-bold ${isActive ? "bg-[#E60012] text-white" : "bg-gray-200 text-gray-400"}`}>
                        {idx < stepIdx ? "✓" : idx + 1}
                      </div>
                      <span className={`text-[10px] md:text-xs text-center ${isActive ? "text-[#E60012] font-medium" : "text-gray-400"}`}>{stepConfig.label}</span>
                      {idx < 3 && <div className={`absolute h-0.5 w-1/2 top-3.5 md:top-4 left-1/2 -z-10 ${isActive ? "bg-[#E60012]" : "bg-gray-200"}`} />}
                    </div>
                  );
                })}
              </div>
