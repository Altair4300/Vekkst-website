            ...
            {mode === "form" && (
              <form onSubmit={handleSubmit} className="p-6 space-y-5">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div><label className="block text-sm font-medium mb-1.5">Phone / WhatsApp</label><input type="tel" value={form.phone} onChange={e => setForm({...form, phone: e.target.value})} className="w-full border rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-amber-400" placeholder="+86 131 2520 4154" /></div>
                ...
                <button type="submit" disabled={submitQuote.isPending} className="w-full bg-amber-400 hover:bg-amber-500 text-black py-3.5 rounded-lg font-medium transition-colors disabled:opacity-50 text-base">{submitQuote.isPending ? <span className="flex items-center justify-center gap-2"><Loader2 className="w-5 h-5 animate-spin" /> Submitting...</span> : "Submit Quote Request"}</button>
