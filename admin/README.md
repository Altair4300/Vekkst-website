...
The built static files are served by `vite preview`. For a production-grade setup, consider serving the `dist/` folder with a static file server or nginx instead.

## Type Safety

The tRPC client imports `AppRouter` from `../../../source/api/router` at build time, giving full end-to-end type safety without duplicating any backend code.
