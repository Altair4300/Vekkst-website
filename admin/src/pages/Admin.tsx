...
// Suppress unused warning — isMainAdmin is kept for parity with the source component
void isMainAdmin;

const EMOJIS = ["😀", "😂", "😍", "🙏", "👍", "👎", "🔥", "❤️", "😊", "🎉", "👏", "🤔", "😭", "😎", "🤝", "✅", "📎", "📸", "🎥", "🎁"];

// The URL of the public website — used for the "Back to Website" link.
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#1a1a2e] to-[#16213e]">
      <div className="bg-white p-10 rounded-xl w-full max-w-sm shadow-2xl">
        <img src="/images/vekkst-logo.png" alt="VEKKST" className="h-10 mx-auto mb-5" />
        ...
        <div className="flex flex-col items-center gap-2 mt-4">
          ...
          <a href={WEBSITE_URL} className="text-xs text-gray-500 hover:text-[#E60012]">
  ...
  return (
    <div className="w-64 bg-[#1a1a2e] text-white fixed top-0 left-0 bottom-0 flex flex-col z-50">
      <div className="p-5 border-b border-white/10 flex items-center gap-3">
        <img src="/images/vekkst-logo.png" alt="VEKKST" className="h-8" />
...
function MessagesPage() {
  ...
  const handleTranslate = async (msgId: number, msgText: string) => {
    if (messageTranslations[msgId]) return;
    setTranslatingIds(prev => ({ ...prev, [msgId]: true }));
    try {
      const result = await translateMutation.mutateAsync({ text: msgText, target: lang });
...
function TeamPage() {
  ...
  const openEditPerms = (s: { id: number; permissions: string | null }) => {
    setEditingId(s.id);
